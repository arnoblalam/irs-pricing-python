...
program define mc_abadie, rclass
	...
	// Some correlation between the pre-period error and X1
	gen double e_pre  = rnormal(0.5*X1, 1)
	gen double e_post = rnormal(0.5*X1, 1)
    ...
    // 3. ESTIMATE ABADIE ATT USING GME (Linear Prob Model) FIRST STAGE
    // ---------------------------------------------------------------
...
twoway (kdensity att_gme, range(0 4)) ///
	...
	title("Estimator performance: Errors Correlated with X1")
	
graph export "reports/figures/estimator_correlaed_X1.png", replace
...
esttab matrix(Perf) using "reports/tables/Correlation_X1_Abadie.html", ///
