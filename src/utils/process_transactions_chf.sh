...
transaction_base="data/processed/CHF"
yield_curve_base="data/raw/curves/CHF_old"
output_base="data/pricing/CHF"
...
# Function to convert date format from YYYY-MM-DD to MMDDYYYY
function convert_date_format_2() {
    echo "$(echo $1 | cut -c1-4)$(echo $1 | cut -c6-7)$(echo $1 | cut -c9-10)"
}
...
for transaction_file in "$transaction_base"/*_output.xlsx; do
    ...
    curve_date_str=$(convert_date_format_2 "$date_str")


    # Set the corresponding yield curve file path
    yield_curve_file="$yield_curve_base/CHF_${curve_date_str}.xlsx"

    # Set the output file path
    output_file="$output_base/CHF_${date_str}_priced.xlsx"

    # Set the evaluation date
    evaluation_date="$date_str"

    # Run the price_swaps.py script with the required arguments
    ./src/price_swaps_chf.py "$transaction_file" "$yield_curve_file" "$output_file" "$evaluation_date"
done
