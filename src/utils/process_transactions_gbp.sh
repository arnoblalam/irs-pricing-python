...
transaction_base="data/processed/GBP"
yield_curve_base="data/raw/curves/GBP"
output_base="data/pricing/GBP"
...
for transaction_file in "$transaction_base"/*_output.xlsx; do
    ...
    yield_curve_file="$yield_curve_base/GBP_${curve_date_str}.xlsx"

    # Set the output file path
    output_file="$output_base/GBP_${date_str}_priced.xlsx"

    # Set the evaluation date
    evaluation_date="$date_str"

    # Run the price_swaps.py script with the required arguments
    ./src/price_swaps_gbp.py "$transaction_file" "$yield_curve_file" "$output_file" "$evaluation_date"
done
