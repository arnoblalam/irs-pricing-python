capture program drop noisy_case
program define noisy_case, rclass
    version 17.0
    ...
    /* ***  noisier selection  *** */
    gen u       = rnormal()                   // extra noise
    gen linpred = 0.25*X1 - 0.4*X2 + 0.8*u    // both flatter & noisy
...
simulate att_logit=r(att_logit) att_gme=r(att_gme) att_classical=r(att_classical), reps(5000): noisy_case
...
twoway (kdensity att_classical) ///
	   ...
	   title("ATT Estimators (noisy case)")
