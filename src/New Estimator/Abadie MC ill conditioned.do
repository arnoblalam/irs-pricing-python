...
program define mc_abadie, rclass
    ...
    gen Ypre  = 1 + 0.3*X1 + 0.3*X2 + 0.2*D + X3 + e_pre
    gen Ypost = 3 + 0.3*X1 + 0.3*X2 + (1.5 + 0.5*X1)*D + X3 + e_post
    ...
    // 3. ESTIMATE ABADIE ATT USING OLS (Linear Prob Model) FIRST STAGE
    // ---------------------------------------------------------------
...
twoway (kdensity att_gme, range(0 4)) ///
	...
	title("Estimator Performance: Ill-Condtioned Design Matrix")
	
graph export "reports/figures/estimator_ill_conditioned.png", replace
...
esttab matrix(Perf) using "reports/tables/Ill_Conditioned_Abadie.html", ///
