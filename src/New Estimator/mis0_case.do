capture program drop miss0_case
program define miss0_case, rclass
    version 17.0
    ...
    /* *** true selection model has X^2 terms *** */
    gen linpred_true = 0.5*X1 - 0.8*X2 + 0.4*X1^2 - 0.3*X2^2
    gen pscore  = invlogit(linpred_true)
...
simulate att_logit=r(att_logit) att_gme=r(att_gme) att_classical=r(att_classical), reps(5000): miss0_case
...
twoway (kdensity att_classical) ///
	   ...
	   title("ATT Estimators (mis-spec case)")
