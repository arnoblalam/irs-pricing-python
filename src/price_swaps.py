...
def add_historical_fixings(libor_index):
    # Replace this example data with your actual historical fixings data
    historical_fixings = [
        ...
        # Add more historical fixings as needed
    ]

    for fixing_date_str, fixing_rate in historical_fixings:
        fixing_date = ql.DateParser.parseFormatted(fixing_date_str, "%Y-%m-%d")
        libor_index.addFixing(fixing_date, fixing_rate, True)
...
def build_helpers(yield_curve_df):
    ...
    for _, row in yield_curve_df.iterrows():
        ...
        if 'Index' in description:
            rate_helpers.append(ql.DepositRateHelper(ql.QuoteHandle(ql.SimpleQuote(rate/100)), ql.Period(tenor), 2, ql.UnitedStates(), ql.ModifiedFollowing, False, ql.Actual360()))
            ...
            rate_helpers.append(ql.SwapRateHelper(ql.QuoteHandle(ql.SimpleQuote(rate/100)), ql.Period(tenor), ql.UnitedStates(), ql.Annual, ql.Unadjusted, ql.Thirty360(ql.Thirty360.USA), ql.USDLibor(ql.Period('3M'))))
...
def price_swaps(transactions_df, yield_curve, index):
    results = []
    for _, row in transactions_df.iterrows():
        try:
            ...
            fixed_rate /= 100 if rate_1 > 10 or rate_2 > 10 else 1

            float_rate = rate_1 if leg_1 != 'FIXED' else rate_2
            if pd.isnull(float_rate):
                float_rate = 0
            float_rate /= 10000  if float_rate > 10 else 100
            ...
            fixed_schedule = ql.Schedule(effective_date, maturity_date, fixed_leg_frequency, ql.UnitedStates(), ql.ModifiedFollowing, ql.ModifiedFollowing, ql.DateGeneration.Forward, False)
            float_schedule = ql.Schedule(effective_date, maturity_date, float_leg_frequency, ql.UnitedStates(), ql.ModifiedFollowing, ql.ModifiedFollowing, ql.DateGeneration.Forward, False)

            swap = ql.VanillaSwap(ql.VanillaSwap.Payer, notional, fixed_schedule, fixed_rate, ql.Thirty360(ql.Thirty360.USA), float_schedule, index, float_rate, index.dayCounter())
            ...
            logging.info(f"Error processing swap: {row}\nError: {str(e)}")
            continue
...
def main():
    ...
    # Add historical fixings to the USD Libor 3M index
    add_historical_fixings(index)
