...
did_model_advanced <- lm(
  Difference ~ Group * Period + I(Tenure^2) + I(Ln_notional^2) + Capped + SEF +
...
stargazer(did_model_advanced, type = "html",
          title = "Difference-in-Differences Regression Results",
          align = TRUE,
          column.labels = c("Advanced Model"),
          ...
          digits = 4,
          no.space = TRUE,
          out = "reports/tables/TABLE DiD (all phases) funky.html")
