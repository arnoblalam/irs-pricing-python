...
program define did_sim, rclass
	...
	* 2. Treatment assignment  ───►  TRUE model is *probit* + a non‑linear term
	tempvar linpred
	gen    `linpred' = 0.5*x1 - 0.3*x2   + 0.7*(x1^2)      // <- quadratic term is *omitted* later
	gen    ps        = normal(`linpred')                   // Φ(·)  =  probit link
	gen    D         = (runiform() < ps)                   // binary treatment draw
...
twoway ///
    ...
    , legend(order(1 "Unweighted DiD" 2 "Weighted DiD" 3 "GME DiD")) ///
      title("Sampling distributions of DiD estimators with mis-specified selection model") ///
...
esttab ., ///
    ...
    label title("Performance of DID Estimators over 1000 Simulations with mis-specified selection model") ///
